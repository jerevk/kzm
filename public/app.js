...
function liveWindowOpen(){
 ...
 return (state.roundData?.fixtures||[]).some(f=>Number(f.api_id)>0&&Number(f.manual_score)!==1&&Number(f.kickoff)>0&&Number(f.kickoff)<=now&&Number(f.kickoff)>=now-4*60*60);
}
...
function sourceRaceHtml(d){
 ...
 if(!rows.length)return `<div class="card"><div class="card-head"><h2>Utrka izvora</h2><span class="pill">TEST</span></div><p class="fine">Usporedba ESPN-a i football-data.org ne utječe na bodovanje. Bilježenje počinje automatski tek kad neka Premier League utakmica stvarno krene.</p><div class="empty">Još nema zabilježenih live promjena.</div></div>`;
 ...
 return `<div class="card"><div class="card-head"><h2>Utrka izvora</h2><span class="pill ${Number(d.sourceRaceActive||0)>0?'green':''}">${Number(d.sourceRaceActive||0)>0?'LIVE TEST':'ČEKA UTAKMICU'}</span></div><p class="fine">Dijagnostika samo promatra. <strong>ESPN ne mijenja rezultate, pickove ni bodove.</strong> Oba izvora se provjeravaju samo dok utakmica traje; tablica pokazuje kada je svaki izvor prvi put prijavio pojedini rezultat. Admin ekran se tada osvježava svakih 20 sekundi.</p>${d.sourceRaceEspnError?`<div class="error">ESPN test: ${esc(d.sourceRaceEspnError)}</div>`:''}<div class="table-wrap"><table><thead><tr><th>Utakmica</th><th>Rezultat</th><th>ESPN</th><th>football-data.org</th><th>Brži izvor</th></tr></thead><tbody>${body||'<tr><td colspan="5">Čekam prvu promjenu rezultata.</td></tr>'}</tbody></table></div></div>`;
}
