...
const pushConfigured=env=>!!(env.ONESIGNAL_APP_ID&&env.ONESIGNAL_API_KEY);
async function pushSend(env,playerIds,title,message,eventKey){
 const ids=[...new Set((playerIds||[]).filter(Boolean).map(String))];
 if(!pushConfigured(env)||!ids.length)return {ok:true,skipped:true};
 const response=await fetch('https://api.onesignal.com/notifications?c=push',{method:'POST',headers:{'Authorization':'Key '+env.ONESIGNAL_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({app_id:env.ONESIGNAL_APP_ID,target_channel:'push',include_aliases:{external_id:ids},headings:{en:title},contents:{en:message},name:String(eventKey||'kzm').slice(0,128),custom_data:{kzm_event:String(eventKey||'')}})});
 const text=await response.text();if(!response.ok)throw new Error('OneSignal HTTP '+response.status+': '+text.slice(0,220));
...
async function oneSignalUserStatus(env,player){
 ...
 let response;
 try{response=await fetch(url,{headers:{'Authorization':'Key '+env.ONESIGNAL_API_KEY,'Accept':'application/json'}});}catch(e){return {playerId:player.id,name:player.name,found:false,error:'OneSignal veza nije odgovorila.'};}
 ...
 return {playerId:player.id,name:player.name,found:true,oneSignalId:data.identity?.onesignal_id||null,enabledPush:enabled.length,totalPush:push.length,types:[...new Set(push.map(x=>String(x?.type||'Push')).filter(Boolean))]};
}
async function pushDiagnostics(env){
 ...
 for(const p of players)rows.push(await oneSignalUserStatus(env,p));
